...
export default function CartPage() {
  ...
  const totalPrice = cartItems.reduce((total, item) => (item.discont_price === null ? total + item.price * item.quantity : total + item.discont_price * item.quantity), 0);
  ...
  return (
    <div className={styles.CartPage}>
      ...
      {cartItems.length === 0 ? (
        <div>
          <p className={styles.CartPage_emptyWarning}>Looks like you have no items in your basket currently.</p>
          <Link className={styles.CartPage_continueBtn} to="/products">
            <div className={styles.CartPage_continueBtn_btn}>Continue shopping</div>
          </Link>
        </div>
      ) : (
                  ...
                  <div className={styles.CartPage_itemInfo_left}>
                    ...
                    <div className={styles.CartPage_itemInfo_leftPrice}>
                      <div className={styles.CartPage_itemInfo_leftBtnCounter}>
                        <button style={{ zIndex: 1 }} onClick={() => handleChangeQuantity('decrement', item.id)}>
                          -
                        </button>
                        <input style={{ position: 'relative', right: '5px' }} type="number" min="1" onChange={(e) => handleChangeInput(item.id, e.target.value)} value={item.quantity} />
                        <button style={{ position: 'relative', right: '10px' }} onClick={() => handleChangeQuantity('plus', item.id)}>
                          +
                        </button>
                      </div>
                      <div className={styles.CartPage_itemInfo_leftPrice_price}>
                        {item.discont_price ? (
                          <>
                            ...
                            <p className={styles.defaultPrice}>${item.price * item.quantity}</p>
                          </>
                        ) : (
                          <p className={styles.discountPrice}>${item.price * item.quantity}</p>
                        )}
                      </div>
                    </div>
                  </div>
                  <div className={styles.deleteButtonBox}>
                    <button className={styles.deleteButton} onClick={() => dispatch(removeItem({ id: item.id }))}>
